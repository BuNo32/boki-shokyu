...
async function loadQuiz(jsonPath, mountId){
  ...
  if(!el){ console.warn("mount element not found:", mountId); return; }
  ...
  data.forEach((q, i) => {
    ...
    if (q.question_type === 'term_mcq' || q.question_type === 'journal_mcq') {
      q.choices?.forEach((ch, idx) => {
        ...
        radio.type = 'radio'; radio.name = `q${i}`; radio.value = ch; radio.id = id;
    ...
    btn.onclick = () => {
      ...
      if (q.question_type.endsWith('mcq')) {
        ...
        const ans = q.answer || [];
        const drOk = ans.some(x => x.side==='Dr' && x.account===drAcc && Number(x.amount)===drAmt);
        const crOk = ans.some(x => x.side==='Cr' && x.account===crAcc && Number(x.amount)===crAmt);
    ...
    qWrap.appendChild(btn); qWrap.appendChild(fb);
